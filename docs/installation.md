...
unsigned), so there is no Gatekeeper "is damaged" alert, and installs bash, zsh
and fish completions generated by the binary itself. Installed any other way,
`proximo completion <shell>` prints the same script.
